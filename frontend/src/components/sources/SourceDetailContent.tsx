...
function SourceDetailContentInner({
  ...
  const isPdfFile = useMemo(() => {
    ...
    const titleIsPdf = source.title?.toLowerCase().endsWith('.pdf')
    const pathIsPdf = source.asset?.file_path?.toLowerCase().endsWith('.pdf')
    return Boolean(titleIsPdf || pathIsPdf)
  }, [source])

  const loadPdfUrl = useCallback(async () => {
    if (!source?.id || pdfUrl || loadingPdf || !isPdfFile) return
    try {
      setLoadingPdf(true)
      setPdfError(null)
      ...
      setPdfError(t('sources.fileUnavailable'))
    } finally {
      setLoadingPdf(false)
    }
  }, [source?.id, pdfUrl, loadingPdf, isPdfFile, t])
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        <Tabs defaultValue="content" className="w-full" onValueChange={(val) => { if (val === 'pdf') void loadPdfUrl() }}>
          <TabsList className="w-full sticky top-0 z-10 bg-card">
            <TabsTrigger value="content">{t('sources.content')}</TabsTrigger>
            {isPdfFile && (
              <TabsTrigger value="pdf" onClick={loadPdfUrl}>
                PDF
              </TabsTrigger>
            )}
          ...
          {isPdfFile && (
            <TabsContent value="pdf" className="mt-5 h-[650px] flex flex-col">
              {loadingPdf ? (
                <div className="flex flex-col items-center justify-center h-full gap-2 p-12">
                  <LoadingSpinner />
                  <span className="text-xs text-muted-foreground">{t('sources.preparing')}</span>
                </div>
              ) : pdfError ? (
                ...
                <div className="flex-1 w-full h-full rounded-md border border-border overflow-hidden bg-zinc-900 min-h-[500px]">
                  <iframe
                    src={pdfUrl}
                ...
                <div className="flex flex-col items-center justify-center h-48 gap-3 border border-dashed rounded-md p-6">
                  <FileText className="h-8 w-8 text-muted-foreground" />
                  <Button variant="outline" onClick={loadPdfUrl}>
                    {t('sources.downloadFile')} (PDF)
                  </Button>
                </div>
              )}
            </TabsContent>
